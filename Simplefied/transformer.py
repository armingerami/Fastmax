...
def fastmax_function(q, k, v, h, normalize=1, temperature=1):
    """
    Optimized version of fast attention approximation.
    ...
    temperature: Controls standard deviation of <q, k>
    Output: Attention result (b, h, n, d)
    """

    # b, n, hd to b,h,n,d
    q = q.reshape((q.shape[0],h,q.shape[1],int(q.shape[2]/h)))
    k = k.reshape((k.shape[0],h,k.shape[1],int(k.shape[2]/h)))
    v = v.reshape((v.shape[0],h,v.shape[1],int(v.shape[2]/h)))

    if normalize == 1:
        q_norm = torch.linalg.norm(q, dim=3, keepdim=True)
        k_norm = torch.linalg.norm(k, dim=3, keepdim=True)
        
        q_inf_norm = torch.linalg.norm(q_norm, dim=2, ord=float('inf'), keepdim=True)
        k_inf_norm = torch.linalg.norm(k_norm, dim=2, ord=float('inf'), keepdim=True)
    ...
    # First term: a0 * sum(v, dim=-2) -> (b, h, d)
    first_term = torch.sum(v, dim=-2)
    
    # Second term: a1 * k^T @ v / temperature -> (b, h, d, d)  
    second_term = torch.matmul(k.transpose(-2, -1), v) * inv_temp
    
    # Compute attention output: q @ second_term -> (b, h, n, d)
    ans = torch.matmul(q, second_term)
    
    ans = ans + first_term.unsqueeze(-2)  # Broadcasting (b,h,d) -> (b,h,1,d) -> (b,h,n,d)

    # b, h, n, d to b, n, hd
    ans = ans.reshape((ans.shape[0],q.shape[2],q.shape[1]*q.shape[3]))
    return ans
...
class Attention(nn.Module):
    def __init__(
        ...
        super().__init__()
        # print(str("Using this Blyat0*#$")*100)
        self.first_it = True
...
class AttentionalPooler(nn.Module):
    def __init__(
        ...
        super().__init__()
        # print(str("Using this Blyat1*#$")*100)
...
class ResidualAttentionBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        # print(str("Using this Blyat3*#$")*100)
        self.ln_1 = norm_layer(d_model)
        self.use_linear_attention = True
        self.n_head = n_head
    ...
    def attention(
        ...
        # print(q_x.shape)
        if self.use_linear_attention: return fastmax_function(q_x, k_x, v_x, self.n_head)
...
class CustomTransformer(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # print(str("Using this Blyat*#$")*100)
        self.width = width
        self.layers = layers
